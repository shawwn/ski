...
window.BezierDrawer = function(canvas, autoTesselates, mode) {
    ...
    canvas.addEventListener("touchstart", genericTouchHandler(canvasDown), false);
	...
	var top_border = parseFloat(style.getPropertyValue('border-top-width'));
	var left_border = parseFloat(style.getPropertyValue('border-left-width'));
	...
	function drawBorder(ctx)
	{
		...
		ctx.fillText("interactive demo", 0, 0);
	}
	...
	function mousePositionForEvent(e)
	{
		...
		return new Point(e.clientX - rect.left - left_border, 
						 e.clientY - rect.top - top_border);
	}
